# Amul Naik

**AI Platform & Data Engineering Lead** — 10+ years architecting production-grade AI platforms, cloud-native data ecosystems, and intelligent automation across GCP, Azure, and AWS.

Currently an Associate Manager at Ernst & Young, leading architecture and technical delivery of an Enterprise AI Engineering Platform on Google Cloud — multi-agent orchestration, production RAG pipelines (Vertex AI, semantic embeddings, vector search), and Knowledge Graph-backed semantic retrieval over enterprise metadata.

## Core capabilities

- **Enterprise AI**: Agentic AI, LLMs, RAG, Knowledge Graphs, Prompt Engineering, Conversational AI
- **Cloud & AI Platforms**: GCP · Azure · AWS, Vertex AI, Cloud Run & GKE, Pub/Sub, Cloud Composer
- **Data Platform Engineering**: BigQuery, dbt Cloud, Dagster, Airbyte, Data Modeling, Data Observability
- **Applied AI & MLOps**: LLM Engineering, MLOps, AI Evaluation, AI Observability, Model Lifecycle
- **Software Engineering**: Python, SQL, REST APIs, cloud-native & event-driven architecture

## Experience

| Role | Company | Period |
|---|---|---|
| Associate Manager | Ernst & Young | 10/2024 – Present |
| GCP Data Platform Engineer | Omnicom Media Group | 06/2023 – 07/2024 |
| Cloud Data Engineer | WNS | 11/2012 – 06/2023 |
| Software Engineer | Datamatics | 10/2010 – 11/2012 |

**Education**: Advanced Management Programme in Business Analytics (AMPBA), Indian School of Business — AI, ML, GenAI, Data Engineering, Decision Analytics. B.Sc. Information Technology, Mumbai University.

## Featured projects

- **[intelligent-data-mesh-framework](https://github.com/amuln90/intelligent-data-mesh-framework)** — Natural-language federated query engine: Streamlit + FastAPI + ChromaDB semantic search + Groq LLM (NL-to-SQL) + Trino + PostgreSQL, cataloged via OpenMetadata
- **[loan-default-risk-pipeline](https://github.com/amuln90/loan-default-risk-pipeline)** — End-to-end loan default prediction: EDA, preprocessing pipeline, model comparison, and a persisted scikit-learn pipeline for inference
- **[cv-classification-segmentation-detection](https://github.com/amuln90/cv-classification-segmentation-detection)** — Transfer learning image classification, VGG19 fine-tuning, UNet nuclei segmentation, and Faster R-CNN object detection
- **[book-recommendation-engine](https://github.com/amuln90/book-recommendation-engine)** — Collaborative-filtering recommender (SVD, user similarity, PCA/t-SNE) on the Book-Crossing dataset
